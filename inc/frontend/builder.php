<?php
/** header desktop **/
if ( ! function_exists( 'neatify_header_builder' ) ) {
    function neatify_header_builder (){
        $header_builder = '';    
        ...
        if ( is_page() ) {
            if ( function_exists('rwmb_meta') ) {
                ...
                if ($metabox_fb != '') {
                    ...
                    $header_builder = neatify_get_option('header_layout');
                }
            } 
        }else{
            $header_builder = neatify_get_option('header_layout');
...
/** header mobile **/
if ( ! function_exists( 'neatify_mobile_builder' ) ) {
    function neatify_mobile_builder (){
        ...
        if ( is_page() ) {
            if ( function_exists('rwmb_meta') ) {
                ...
                if ($metabox_hmb != '') {
                    ...
                    $mobile_builder = neatify_get_option('header_mobile');
                }
            } 
        }else{
            $mobile_builder = neatify_get_option('header_mobile');
...
/** side panel **/
if ( ! function_exists( 'neatify_sidepanel_builder' ) ) {
    function neatify_sidepanel_builder (){

        $panel_builder = neatify_get_option('sidepanel_layout');
...
/** 404 template **/
if ( ! function_exists( 'neatify_404_builder' ) ) {
    function neatify_404_builder (){

        $error_builder = neatify_get_option('page_404');
        ...
        if( !$error_builder ) { ?>
            <div class="error-404 not-found text-center">
                <div class="container">
                    ...
                    <h1><?php wp_kses( _e( '404', 'neatify' ), wp_kses_allowed_html('post')  ); ?></h1>
                    <h2><?php esc_html_e( 'Sorry! Page Not Found!', 'neatify' ); ?></h2>
                    <div class="page-content">
                        <p><?php esc_html_e( 'Oops! The page you are looking for does not exist. Please return to the site’s homepage.', 'neatify' ); ?></p>
                        <?php get_search_form(); ?>
                        <a class="xptf-btn" href="<?php echo esc_url( home_url( '/' ) ); ?>"><?php esc_html_e( 'take me home', 'neatify' ); ?></a>
                    </div>
                </div>
            </div>
        <?php }else{
...
/** footer **/
if ( ! function_exists( 'neatify_footer_builder' ) ) {
    function neatify_footer_builder (){
        $footer_builder = '';    
        ...
        if ( is_page() ) {
            if ( function_exists('rwmb_meta') ) {
                ...
                if ($metabox_fb != '') {
                    ...
                    $footer_builder = neatify_get_option('footer_layout');
                }
            } 
        }else{
            $footer_builder = neatify_get_option('footer_layout');
