...
if(!function_exists('neatify_color_scheme')){
    function neatify_color_scheme(){
	  	$color_scheme = '';

	  	//Heading Color
	  	if( neatify_get_option('heading_color') != '#191717' ){
			$color_scheme = 
			'
			/****Heading Color****/
			h1,
			...
			h4,
			h5,
			h6{ color: '.neatify_get_option('heading_color').';}
				';
		}
		...
		//Button Hover and Dark Color
	  	if( neatify_get_option('btn_hover_dark') != '#191717' ){
			$color_scheme = 
			'
			/****Secondary Color****/
			...
			.xptf-btn:focus{ background: '.neatify_get_option('btn_hover_dark').';}
				';
		}
		...
		if( !empty($color_scheme) ){
	  	...
	  	if( neatify_get_option('main_color') != '#2ab97e' ){
			...
			.woocommerce button.button{ background: '.neatify_get_option('main_color').'; }
			.slide-rev-subtitle {
				background: '.neatify_get_option('main_color').' !important;
			}
			...
			.icon-box-1 .icon-main{background: '.hex2rgba(neatify_get_option('main_color'), 0.1).';}
			.team-3 .team-thumb a:before{background: '.hex2rgba(neatify_get_option('main_color'), 0.8).';}
			.vertical-services-menu a::before{background: linear-gradient(120deg, transparent, '.hex2rgba(neatify_get_option('main_color'), 0.1).', transparent); }
			.projects-grid .projects-box .portfolio-info,
			.projects-grid.style-3 .projects-thumbnail .overlay{background: '.hex2rgba(neatify_get_option('main_color'), 0.9).';}

			/*Background Image*/
			.author-widget_wrapper:before{ background-image: linear-gradient(230deg, '.neatify_get_option('main_color').' -150%, #fff 80%); }
			...
			.woocommerce div.product .woocommerce-tabs ul.tabs li.active a{ border-color: '.neatify_get_option('main_color').'; }

			/*Border Top Color*/
			.woocommerce-message,
			.woocommerce-info{ border-top-color: '.neatify_get_option('main_color').'; }
			...
			.woocommerce div.product p.price,.woocommerce div.product span.price{ color: '.neatify_get_option('main_color').'; }
			...
			.main-navigation ul > li.menu-item-has-children > a:hover:after{
				color:'.neatify_get_option('main_color').' !important;
			}
			...
			.icon_newmain.elementor-widget-icon.elementor-view-default .elementor-icon{
				color:'.neatify_get_option('main_color').';
			}
			...
			.border_clr.elementor-widget-divider.elementor-view-default .elementor-icon svg {
			  fill: '.neatify_get_option('main_color').';
			}
			.service-icon-wrapper i, 
			.service-icon-wrapper svg, 
			.border_clr.elementor-widget-divider.elementor-view-framed .elementor-icon, 
			...
			.border_clr.elementor-widget-divider {
			  --divider-color: '.neatify_get_option('main_color').'; }
			.icon-box .icon-main svg,
			.icon-box-grid .icon-box .icon-main svg,
			.xp-counter-2 svg{ fill: '.neatify_get_option('main_color').'; }
				';
		}
...
add_action('wp_head', 'neatify_color_scheme');

//Custom Second Font
if(!function_exists('neatify_second_font')){
	function neatify_second_font(){
...
add_action('wp_head', 'neatify_second_font');