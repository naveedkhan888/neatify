...
add_action( 'elementor/widgets/register', 'neatify_register_elementor_widgets' );
function neatify_register_elementor_widgets() {
...
// Add a custom 'category_neatify' category for to the Elementor element panel so that our theme's widgets have their own category.
add_action( 'elementor/init', function() {
    \Elementor\Plugin::$instance->elements_manager->add_category( 
        'category_neatify',
        [
            'title' => __( 'Neatify', 'neatify' ),
            'icon' => 'fa fa-plug', //default icon
        ],
        1 // position
    );
    \Elementor\Plugin::$instance->elements_manager->add_category( 
        'category_neatify_header',
        [
            'title' => __( 'XP Header', 'neatify' ),
            'icon' => 'fa fa-plug', //default icon
        ],
        2 // position
    );
...
function neatify_add_cpt_support() {
...
add_action( 'elementor/init', 'neatify_add_cpt_support' );

// Upload SVG for Elementor
function neatify_unfiltered_files_upload() {
...
add_action( 'elementor/init', 'neatify_unfiltered_files_upload' );



/*Fix Elementor Pro*/
function neatify_register_elementor_locations( $elementor_theme_manager ) {

    $elementor_theme_manager->register_all_core_location();

}
add_action( 'elementor/theme/register_locations', 'neatify_register_elementor_locations' );
...
add_action('elementor/element/container/section_layout/after_section_end', function( $container, $args ) {
    ...
    $container->add_control(
        'sticky_class',
        [
            'label'        => __( 'Sticky On/Off', 'neatify' ),
    ...
    $container->add_control(
        'sticky_background',
        [
            'label'     => __( 'Background Scroll', 'neatify' ),
